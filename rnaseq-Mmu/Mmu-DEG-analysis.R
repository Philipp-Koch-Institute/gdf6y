...
### Annotation loading ###
ah <- AnnotationHub()
ah.query <- query(ah, c("EnsDb", "Mus musculus", ""))
...
# download.file(url = "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE233nnn/GSE233995/suppl/GSE233995%5Fcounts%2Ecsv%2Egz",destfile = "GSE233995_counts.csv.gz")
# for the moment, read-in the provided csv file.
data.df.counts <- read.table(file = "GSE233995_counts.csv.gz", sep = ",", header = TRUE)
...
write.xlsx(list("A) Meta data" = metadata,
                ...
                "D) gdf6Y vs del9" = res.ordered.df.2), file = paste0("Supplementary_Data_2_ARichter2024.xlsx"), rowNames = FALSE)
